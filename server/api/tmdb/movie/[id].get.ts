export default defineEventHandler(async (event) => {
  ...
  const url = `https://api.themoviedb.org/3/movie/${id}?api_key=${apiKey}&append_to_response=credits,images,recommendations,similar`
  ...
  try {
    ...
    throw createError({
      statusCode: 500,
      statusMessage: `Failed to fetch movie details: ${error.message}`
    })
  }
})