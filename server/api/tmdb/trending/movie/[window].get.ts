export default defineEventHandler(async (event) => {
  ...
  const url = `https://api.themoviedb.org/3/trending/movie/${window}?api_key=${apiKey}&page=${query.page || 1}`
  ...
  try {
    ...
    throw createError({
      statusCode: 500,
      statusMessage: `Failed to fetch trending movies: ${error.message}`
    })
  }
})